...
public class RegionMap {
    ...
    public void removeFloor(int plane, int x, int y) {
        removeMask(plane, x, y, Flags.FLOOR_BLOCKSWALK);
    }
    ...
    public void addWall(int plane, int x, int y, int type, int rotation, boolean solid, boolean notAlternative) {
        ...
        /*
         * if (notAlternative) { if (type == 0) { if (rotation == 0) {
         * addMask(plane, x, y, 0x20000000); addMask(plane, x - 1, y,
         * 0x2000000); } if (rotation == 1) { addMask(plane, x, y, 0x800000);
         * addMask(plane, x, y + 1, 0x8000000); } if (rotation == 2) {
         * addMask(plane, x, y, 0x2000000); addMask(plane, x + 1, y,
         * 0x20000000); } if (rotation == 3) { addMask(plane, x, y, 0x8000000);
         * addMask(plane, x, y - 1, 0x800000); } } if (type == 1 || type == 3) {
         * if (rotation == 0) { addMask(plane, x, y, 0x400000); addMask(plane, x
         * - 1, y + 1, 0x4000000); } if (rotation == 1) { addMask(plane, x, y,
         * 0x1000000); addMask(plane, 1 + x, 1 + y, 0x10000000); } if (rotation
         * == 2) { addMask(plane, x, y, 0x4000000); addMask(plane, x + 1, -1 +
         * y, 0x400000); } if (rotation == 3) { addMask(plane, x, y,
         * 0x10000000); addMask(plane, -1 + x, y - 1, 0x1000000); } } if (type
         * == 2) { if (rotation == 0) { addMask(plane, x, y, 0x20800000);
         * addMask(plane, -1 + x, y, 0x2000000); addMask(plane, x, 1 + y,
         * 0x8000000); } if (rotation == 1) { addMask(plane, x, y, 0x2800000);
         * addMask(plane, x, 1 + y, 0x8000000); addMask(plane, x + 1, y,
         * 0x20000000); } if (rotation == 2) { addMask(plane, x, y, 0xa000000);
         * addMask(plane, 1 + x, y, 0x20000000); addMask(plane, x, y - 1,
         * 0x800000); } if (rotation == 3) { addMask(plane, x, y, 0x28000000);
         * addMask(plane, x, y - 1, 0x800000); addMask(plane, -1 + x, y,
         * 0x2000000); } } }
         */
    }
    ...
    public void removeWall(int plane, int x, int y, int type, int rotation, boolean solid, boolean notAlternative) {
        ...
        /*
         * if (notAlternative) { if (type == 0) { if (rotation == 0) {
         * removeMask(plane, x, y, 0x20000000); removeMask(plane, x - 1, y,
         * 0x2000000); } if (rotation == 1) { removeMask(plane, x, y, 0x800000);
         * removeMask(plane, x, y + 1, 0x8000000); } if (rotation == 2) {
         * removeMask(plane, x, y, 0x2000000); removeMask(plane, x + 1, y,
         * 0x20000000); } if (rotation == 3) { removeMask(plane, x, y,
         * 0x8000000); removeMask(plane, x, y - 1, 0x800000); } } if (type == 1
         * || type == 3) { if (rotation == 0) { removeMask(plane, x, y,
         * 0x400000); removeMask(plane, x - 1, y + 1, 0x4000000); } if (rotation
         * == 1) { removeMask(plane, x, y, 0x1000000); removeMask(plane, 1 + x,
         * 1 + y, 0x10000000); } if (rotation == 2) { removeMask(plane, x, y,
         * 0x4000000); removeMask(plane, x + 1, -1 + y, 0x400000); } if
         * (rotation == 3) { removeMask(plane, x, y, 0x10000000);
         * removeMask(plane, -1 + x, y - 1, 0x1000000); } } if (type == 2) { if
         * (rotation == 0) { removeMask(plane, x, y, 0x20800000);
         * removeMask(plane, -1 + x, y, 0x2000000); removeMask(plane, x, 1 + y,
         * 0x8000000); } if (rotation == 1) { removeMask(plane, x, y,
         * 0x2800000); removeMask(plane, x, 1 + y, 0x8000000); removeMask(plane,
         * x + 1, y, 0x20000000); } if (rotation == 2) { removeMask(plane, x, y,
         * 0xa000000); removeMask(plane, 1 + x, y, 0x20000000);
         * removeMask(plane, x, y - 1, 0x800000); } if (rotation == 3) {
         * removeMask(plane, x, y, 0x28000000); removeMask(plane, x, y - 1,
         * 0x800000); removeMask(plane, -1 + x, y, 0x2000000); } } }
         */
    }
    ...
    public void setMask(int plane, int x, int y, int mask) {
        if (x >= 64 || y >= 64 || x < 0 || y < 0) {
            ...
            if (clipedOnly)
                World.getRegion(tile.getRegionId(), false).forceGetRegionMapClipedOnly().setMask(plane,
                    tile.getX() - newRegionX, tile.getY() - newRegionY, mask);
            else
                World.getRegion(tile.getRegionId(), false).forceGetRegionMap().setMask(plane,
                    tile.getX() - newRegionX, tile.getY() - newRegionY, mask);
            return;
        }
    ...
    public void addMask(int plane, int x, int y, int mask) {
        if (x >= 64 || y >= 64 || x < 0 || y < 0) {
            ...
            int newRegionY = (regionId & 0xff) * 64;
            if (clipedOnly)
                World.getRegion(tile.getRegionId(), false).forceGetRegionMapClipedOnly().addMask(plane,
                    tile.getX() - newRegionX, tile.getY() - newRegionY, mask);
            else
                World.getRegion(tile.getRegionId(), false).forceGetRegionMap().addMask(plane,
                    tile.getX() - newRegionX, tile.getY() - newRegionY, mask);
            return;
        }
    ...
    public void removeMask(int plane, int x, int y, int mask) {
        if (x >= 64 || y >= 64 || x < 0 || y < 0) {
            ...
            int newRegionY = (regionId & 0xff) * 64;
            if (clipedOnly)
                World.getRegion(tile.getRegionId(), false).forceGetRegionMapClipedOnly().removeMask(plane,
                    tile.getX() - newRegionX, tile.getY() - newRegionY, mask);
            else
                World.getRegion(tile.getRegionId(), false).forceGetRegionMap().removeMask(plane,
                    tile.getX() - newRegionX, tile.getY() - newRegionY, mask);
            return;
        }
        masks[plane][x][y] &= (~mask);
