...
public class SergeantGrimspike extends NPCAttackLayout {
    ...
    public void executeAttacks(NPC npc, Mob mainTarget) {
        ...
        Projectiles rangeProj = Projectiles.create(npc.getPosition(), mainTarget.getPosition(), mainTarget, 1206, 32,
            80, 50, 40, 34);
    ...
    public boolean isWithinRadius(NPC npc, Mob target) {
        return !TileControl.locationOccupied(npc, target) && npc.getPosition().isWithinRadius(target, 7);
    }
    ...
    public Hit getDamage(NPC npc, Mob target) {
        ...
        if (target.isPlayer()) {
            ...
            if (npc.getCombatState().getCombatType().equals(CombatType.RANGED)) {
