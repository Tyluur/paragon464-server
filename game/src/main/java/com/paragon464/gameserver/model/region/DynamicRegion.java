            ...
            for (int dynX = 0; dynX < 8; dynX++) {
                for (int dynY = 0; dynY < 8; dynY++) {
                    ...
                    int landArchiveId = Cache.getCacheFileManagers()[5]
                        .getContainerId("l" + (mapID >> 8) + "_" + (mapID & 0xFF));
                    int mapArchiveId = Cache.getCacheFileManagers()[5]
                        .getContainerId("m" + (mapID >> 8) + "_" + (mapID & 0xFF));
                    ...
                    if (landContainerData != null) {
                        ...
                        while ((incr = landStream.readSmart2()) != 0) {
                            ...
                            while ((incr2 = landStream.readUnsignedSmart()) != 0) {
                                ...
                                if (realZ == renderChunkZ && (x >> 3) == renderLocalChunkX
                                    && (y >> 3) == renderLocalChunkY) {
                                    CachedObjectDefinition definition = CachedObjectDefinition.forId(objectId);
                                    int[] coords = translate(x & 0x7, y & 0x7, rotation, definition.sizeX,
                                        definition.sizeY, rot);
                                    Position loc = new Position((dynX << 3) + coords[0] + ((getId() >> 8) << 6),
                                        (dynY << 3) + coords[1] + ((getId() & 0xFF) << 6), dynZ);
                                    GameObject object = new GameObject(loc, objectId, type, (rotation + rot) & 0x3);
                                    spawnObject(object, dynZ, (dynX << 3) + coords[0], (dynY << 3) + coords[1], true);
                                }
                            }
                        }
                    }
                    /*
                     * if (Config.DEBUG && landContainerData == null &&
                     * landArchiveId != -1 && MapArchiveKeys.getMapKeys(mapID)
                     * != null) Logger.log(this, "Missing xteas for region " +
                     * mapID + ".");
                     */
                }
            }
        ...
        for (int x = 0; x < 8; x++) {
            for (int y = 0; y < 8; y++) {
                ...
                for (GameObject removed : ro)
                    if (removed.getPlane() == chunkZ && removed.getPosition().getZoneX() == chunkX
                        && removed.getPosition().getZoneY() == chunkY)
                        removedOriginalObjects.remove(removed);
                /*
                 * for (WorldObject added : ro) if (added.getPlane() == chunkZ
                 * && added.getZoneX() == chunkX && added.getZoneY() ==
